...
InlineCurrentSense::InlineCurrentSense(float _shunt_resistor, float _gain, int _pinA, int _pinB, int _pinC){
    ...
    // volts_to_amps_ratio = 1000.0f / _shunt_resistor; // mV to amps
    // // gains for each phase
    // gain_a = volts_to_amps_ratio;
    // gain_b = volts_to_amps_ratio;
    // gain_c = volts_to_amps_ratio;
...
PhaseCurrent_s InlineCurrentSense::getPhaseCurrents(){
    ...
    // current.a = (!_isset(pinA)) ? 0 : _readADCVoltageInline(pinA, params);// amps
    // current.b = (!_isset(pinB)) ? 0 : _readADCVoltageInline(pinB, params);// amps
    // current.c = (!_isset(pinC)) ? 0 : _readADCVoltageInline(pinC, params); // amps
